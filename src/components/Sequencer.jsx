...
export default function Sequencer(props) {
    ...
    const [data, setData] = createSignal(Array(8).fill(0)); //setData should be prev => {copy ...array, set index, return copied array}
    ...
    const [intervalTime, setIntervalTime] = createSignal((60000.00 / (state.tempo * props.seqInfo.division)));
    var nextBeat = performance.now();
    //var stepIntervalTime = (60000.00 / (state.tempo * props.seqInfo.division));
    createEffect(() => { //listen to when seqinfo in track changes and update timing of sequencer accordingly
        // console.log("division changed");
        const {division} = props.seqInfo;
        
        setIntervalTime((60000.00 / (state.tempo * division)));
        
            
    });
    createEffect(() => { // per row sequencer 
        if(!state) return;
        ...
        <div class={styles.sequencer} ref={parent}>
            <For each={Array.from({length:8}, (_,i) => i + 1)}>
                {(step) => 
                {
                    ...
                    <div style={{width: "12.5%", "align-items": "center", "justify-content": "center", "display": 'flex'}}>
                        <button isOn={step_.on} onclick={() => setStep_("on", !step_.on)} classList={{[styles.step]: true, "step": true}} style={{"background-color": step_.on ? 'rgb(154 208 255)' : 'var(--background-color)'}}></button>
